    ...
    <div className="h-full overflow-y-auto custom-scrollbar bg-[#07080d] px-6 py-6">
      <div className="mx-auto max-w-[1300px] space-y-5">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className={`rounded-xl border px-4 py-3 text-xs font-semibold ${status.isConnected ? 'border-emerald-400/25 bg-emerald-500/10 text-emerald-200' : 'border-rose-400/25 bg-rose-500/10 text-rose-200'}`}>
            {status.isLoading ? 'Checking Ollama...' : status.isConnected ? 'Ollama online' : 'Ollama offline'}
          </div>
        </div>

        <section className="grid gap-4 lg:grid-cols-[1fr_1.2fr]">
          <div className="rounded-xl border border-white/10 bg-[#0d0f16] p-5">
            ...
            <button
              ...
              className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-semibold text-black transition hover:bg-cyan-200 disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/30"
            >
