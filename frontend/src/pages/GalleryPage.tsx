  ...
  return (
    <div className="h-full overflow-y-auto custom-scrollbar bg-[#07080d] px-6 py-6">
      <div className="mx-auto max-w-[1500px] space-y-5">
        ...
        {visible.length === 0 ? (
          <div className="flex h-[58vh] items-center justify-center rounded-xl border border-white/10 bg-black/30 text-slate-500">
            No outputs yet.
          </div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {visible.map((item, idx) => (
              <article key={`${item.url}-${idx}`} className="overflow-hidden rounded-xl border border-white/10 bg-black/30">
