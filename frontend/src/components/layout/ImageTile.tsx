...
export const ImageTile = ({ title, description, Icon, image, themeId = 'midnight', onClick }: ImageTileProps) => {
  const themeStyles = {
    ...
    amber: {
      ...
      overlay: 'from-black/20 via-amber-950/50 to-black/85',
      iconBg: 'bg-black/60 border-amber-900/40',
    },
    teal: {
      border: 'border-teal-900/30',
      overlay: 'from-black/15 via-teal-950/40 to-black/80',
      iconBg: 'bg-black/50 border-teal-800/50',
    },
    violet: {
      border: 'border-violet-900/30',
      overlay: 'from-black/15 via-violet-950/45 to-black/82',
      iconBg: 'bg-black/50 border-violet-800/50',
    },
  }[themeId] ?? {
    border: 'border-white/10',
    overlay: 'from-black/20 via-black/45 to-black/80',
  ...
  return (
    <button
      onClick={onClick}
      className={`group relative w-full h-full overflow-hidden rounded-2xl border ${themeStyles.border} bg-[#0a0f1e] focus:outline-none focus:ring-2 focus:ring-white/40 transition-all active:scale-[0.985]`}
    >
      {/* Background image */}
      <div
      ...
      {/* Icon "sign" in corner */}
      <div className={`absolute bottom-3 right-3 z-10 flex h-10 w-10 items-center justify-center rounded-full border ${themeStyles.iconBg} text-white transition-all group-hover:scale-110`}>
